...
//            dd('hasManyPhx', $localKey, $foreignKey);
    ...
    {
//dd($property,'get' . Helper::pascalCase($property) . 'Attr');
        if (method_exists($this, $property)) {
            // $user->profile 时调用 profile 中定义的模型
            $this->{$property}();
            // 定义获取器
        } elseif (method_exists($this, 'get' . Helper::pascalCase($property) . 'Attr')) {
            $method = 'get' . Helper::pascalCase($property) . 'Attr';
            return $this->{$method}();
        }
