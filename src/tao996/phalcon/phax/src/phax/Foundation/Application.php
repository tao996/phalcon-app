...
class Application
{
    ...
    public static function di(): FactoryDefault
    {
        ...
        if (is_null($_di)) {
            if (IS_WEB) {
                ...
                die('unknown php runtime, create di service failed');
            }
        }
        return $_di;
    }
    ...
    public function __construct(string $sourceRoot)
    {
        if (!file_exists($sourceRoot)) {
            die('could not find the source root path： /xxx/src => /var/www');
        }
    }
...
//            dd($class,$params);
    ...
    public static function addWebServices(FactoryDefault $di): void
    {
        ...
        $di->setShared('cookies', function () {
            $cc = \config('cookie');
            if ($cc['key']) {
                return new \Phalcon\Http\Response\Cookies(true, $cc['key']);
            } else {
    ...
    public function runWeb()
    {
        ...
        try {
            ...
            if (class_exists($errClass)) {
                ...
                die('could not find the error handle class in app.error');
            }
        }
    }
