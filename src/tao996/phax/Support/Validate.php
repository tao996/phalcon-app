...
class Validate
{
    ...
    public static function getCallerValidation(string $rule, array $params): array
    {
        switch (strtolower($rule)) { // 全部都是小写
            ...
            case 'accept':
                return ['accepted', \Phax\Filter\Validation\AcceptedValidation::class];
            case 'after':
                return ['after', \Phax\Filter\Validation\AfterValidation::class,
                    ['date' => $params[0]]
                ];
            case 'alnum': // 字母数字字符
            case 'alphanum':
            ...
            case 'before':
                return ['before', \Phax\Filter\Validation\BeforeValidation::class,
                    ['date' => $params[0]],
                ];
            ...
            case 'boolean':
            case 'bool':
                return ['bool', \Phax\Filter\Validation\BoolValidation::class];
            ...
            case 'neq':
                return ['different', \Phax\Filter\Validation\DifferentValidation::class,
                    'with' => $params[0]
                ];
            ...
            case 'expire': // 验证当前操作（注意不是某个值）是否在某个有效日期之内
                return ['expire', \Phax\Filter\Validation\ExpireValidation::class,
            ...
            case 'idcard':
            case 'card':
                return ['idCard', \Phax\Filter\Validation\IdCardValidation::class];
            case 'in':
            case 'inclusionin':
            ...
            case 'integer':
            case 'int':
                return ['int', \Phax\Filter\Validation\IntValidation::class];
            case 'ip':
            ...
            case 'mac':// mac 地址
                return ['mac', \Phax\Filter\Validation\MacValidation::class];


            case 'notbetween':
                return ['notBetween', \Phax\Filter\Validation\NotBetweenValidation::class,
                    array_combine(['min', 'max'], $params)
                ];
            ...
            case 'zip': // 6 位数的邮政编号
                return ['zip', \Phax\Filter\Validation\ZipValidation::class];
...
//                    $arguments['field'] = $item['title'];
    ...
    {
        if (!empty($phone)) {
            return \Phax\Filter\Validation\MobileCnValidation::match($phone);
        }
        return false;
    }
