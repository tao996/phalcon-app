...
class AnnotationDocCommentParse
{
    ...
    protected function parseAnnotations($str) // str: "\@controller 菜单管理"
    {
        $annotations = [];
        preg_match_all($this->dataPattern, $str, $found);
        foreach ($found[2] as $key => $value) {
            $annotations[$this->sanitizeKey($found[1][$key])][] = $this->dynamicTypeParse(trim($value), $found[1][$key]);
        }
        return $annotations;
    }
    ...
    protected function dynamicTypeParse($value, $annotation = null)
    {
