...
//        'order' => [],
    ...
    public static function with(string|\Phax\Mvc\Model $model, bool $softDelete = true): QueryBuilder
    {
        if (is_string($model)) {
     ...
     * @return string 绑定参数的名称 xxx
     * @throws \Exception
     */
    private function takeBindKey(string $condition): string
    {
        preg_match('|:(\w+):|', $condition, $match);
        if (empty($match[1])) {
    ...
    public function condition(string $condition, mixed $value = null, int $type = \PDO::PARAM_STR): static
    {
        ...
        if (!is_null($value)) {
            $name = $this->takeBindKey($condition);
     ...
     * @param bool $allowEmpty 是否允许为空，如果为 false，则会跳过空值
     * @return $this
     */
    public function int(string $name, string|int $value, bool $allowEmpty = false): static
    {
        $v = intval($value);
        if ($v === 0 && !$allowEmpty) {
            return $this;
...
//        dd($this->builder()->getQuery()->getSql());
    ...
    public function findColumn(array|string $fields, string $key = null): array
    {
        $this->columns($fields);
        $rows = $this->find();
        return $key ? Data::reGroupWithKey($rows, $key) : $rows;
    }
