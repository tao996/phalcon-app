...
class Router
{
     ...
     * @param array $options ['module'=>多模块标识，默认为 m,'project'=> 前端应用 app.project，默认为空]
     * @throws \Exception
     */
...
//            $url = rtrim($url, '/');
            ...
            if (isset($urlElements[1])) {
                $urlElements[1] = Data::formatName($urlElements['1']);
            }
            $data = [
...
//            $m[0] = Data::formatName($m[0]);
//            $m[1] = Data::formatName($m[1]);
    ...
    public static function addRoute(array &$config): void
    {
        ...
        if (!$config['isApi']) {
            if (isset($config['vendor']) && 'overwrite' == $config['vendor']) {
                // TODO register the view in the Module.php by your self
            } else {
...
//      dd(__LINE__,config('app.project'), $config);
     ...
     * @param array $options 配置信息  ['module'=>多模块标识，默认为 m,'project'=> 前端应用 app.project，默认为空]
     * @return array
     * @throws \Exception
     */
...
//        dd($config);
    ...
    public static function formatName(string $name, bool $lcfirst = true): string
    {
        ...
        return Data::formatName($name, $lcfirst);
    }
    ...
    {
        $cName = self::formatName($controller);
        $aName = self::formatName($action);
        return $cName . '/' . $aName;
    }
    ...
    public static function getPickView(): string
    {
        $config = self::$options;
        if (isset($config['subc'])) { // 子目录
            return $config['subc'] . '.' . self::formatPickView($config['pathsname']['controller'], $config['pathsname']['action']);
        } else {
            return $config['pathsname']['controller'] . '/' . $config['pathsname']['action'];
        }
    }

    public static function changePickView($controller, $action)
    {
        self::$options['pickview'] = self::formatPickView($controller, $action);
