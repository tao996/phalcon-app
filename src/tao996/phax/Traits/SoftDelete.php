...
trait SoftDelete
{
    ...
    {
        $this->{$this->deletedTime} = \Phax\Behavior\Model::printTimestampFormat($this->autoWriteTimestamp);
        return $this->update();
    }
     ...
     * findFirst() // 查询最后一条记录 id DESC
     * findFirst(5) // 数字 查询主键 id=5 的记录
     * findFirst('name="phx"') // 字符串，直接设置条件
     * findFirst(['name'=>'phx', 'age'=>5]) // 使用绑定方式查询 name='phx' AND age=5 的记录
     * </pre>
     * @param callable|null $queryBuilder \Phalcon\Mvc\Model\Query\Builder()
     * @throws \Exception
     */
    public static function findFirst($parameters = null, callable $queryBuilder = null): null|\Phalcon\Mvc\Model|self
    {
        /**
...
//        dd($results,$results->getFirst());
    ...
    public static function __callStatic(string $method, array $arguments)
    {
        if (str_starts_with($method, 'findBy')) {
            $name = Helper::uncamelize(substr($method, 6), '_');
            return self::find([
                $name . '= :key:', 'bind' => ['key' => $arguments[0]]
            ]);
        } elseif (str_starts_with($method, 'findFirstBy')) {
            $name = Helper::uncamelize(substr($method, 11), '_');
