...
class SystemUser extends BaseModel
{
    ...
    public int $status = Config::STATUS_ACTIVE; // 状态:1启用
    ...
    public function newAccount($account): void
    {
        if (Validate::isEmail($account)) {
            $this->email = $account;
            $this->email_at = time();
            $this->email_valid = Config::STATUS_ACTIVE;
        } elseif (Validate::isPhone($account)) {
            $this->phone = $account;
            $this->phone_at = time();
            $this->phone_valid = Config::STATUS_ACTIVE;
        }
    }
    ...
    {
        return in_array($this->id, Config::SUPER_ADMIN_IDS);
    }

    public function hasLoginAccount(): bool
    {
        if ($this->phone_valid == Config::STATUS_ACTIVE && !empty($this->phone)) {
            return true;
        }
        if ($this->email_valid == Config::STATUS_ACTIVE && !empty($this->email)) {
            return true;
        }
        return false;
    }
    ...
    public function beforeCreate()
    {
        ...
        if (empty($this->seed)) {
            $this->seed = Helper::random(); // 默认 字母数字，长度8
        }
    }
