...
const admin = {
    ...
    util: {
        ...
        /**
         * @param {Object} binds 绑定操作 {name:function(othis){}}
    ...
    storage: {
        ...
        getArray: function (name, remove = true) {
        ...
        getFirst: function (name, remove = true) {
            const rows = this.getArray(name, remove);
            return rows && rows.length > 0 ? rows[0] : null;
        },
        getParse: function (name, remove = true) {
            const data = localStorage.getItem(name);
            if (remove) {
                localStorage.removeItem(name);
            }
                        ...
                        admin.iframe.open(
                            admin.config.url.imageList + '?type=' + selectCheck, {
                                title: '图片选择',
                                    ...
                                    const images = admin.storage.getArray('images');
                                    if (images) {
