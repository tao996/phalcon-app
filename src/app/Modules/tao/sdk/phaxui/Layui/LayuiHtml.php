...
class LayuiHtml
{
    ...
    ])
    {
        ...
        $tipDiv = $options['tip'] ? '<div class="hint">' . $options['tip'] . '</div>' : '';
        echo <<<HTML
<div class="btn-image-upload {$options['class']}" style="margin-bottom: 10px;">
...
<a style="margin-left: 6px;" class="layui-btn layui-btn-normal" id="select_{$name}"
...
HTML;
    ...
    /**
     * 设置面包屑导航
     * @param array|string $menu 需要添加的菜单，格式为一维数组的 ['href'=>'链接','text'=>标题] 或者 ['标题'] 或者 [['href'=>'x1','text'=>'A1'], ['href'=>'x2','text'=>'A2']]
     * @param bool $output 是否输出模板，默认为 false（将 $menu 添加到菜单）
     * @return string
     */
    public static function breadcrumb(array|string $menu = [], bool $output = false): string
    {
        static $items = [];
        if ($output) {
            if ($items) {
            ...
            if ($menu) {
                // 格式检查
                if (is_string($menu)) {
                    $items[] = ['href' => '', 'text' => $menu];
                } elseif (isset($menu[0]) && is_string($menu[0])) {
                    $items[] = ['href' => '', 'text' => $menu[0]];
