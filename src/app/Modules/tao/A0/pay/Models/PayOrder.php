...
namespace app\Modules\tao\A0\pay\Models;
...
class PayOrder extends BaseModel
{
    ...
    {
        $this->rndcode = Helper::random(0, 5);
    }
    ...
    public static function getIntTradeType(string $trade_type): int
    {
        if (isset(self::MapTradeType[$trade_type])) {
            return self::MapTradeType[$trade_type];
        } else {
            Logger::info('未能找到匹配的交易类型:' . $trade_type);
            return 0;
        }
    }


    public static function getIntStatus(string $status): int
    {
        if (isset(self::MapStatus[$status])) {
            return self::MapStatus[$status];
        } else {
            Logger::info('未能找到匹配的订单状态:' . $status);
     ...
     * @throws \Exception
     */
    public static function fromOutTradeNo(string $outTradeNo, array $columns = []): PayOrder
    {
