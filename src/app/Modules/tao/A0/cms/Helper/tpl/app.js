/**
 * 返回实现
 * @param {{id:string,title:string, prefix:string, methods?:Object, key?:string, doData?:function}} options 配置信息
...
function vueArray(options, items = []) {
    ...
    const app = Vue.createApp({
        ...
        methods: {
            editWith(ii) { // 编辑记录
                ...
                admin.iframe.open(options.prefix + '/edit?storage=true&id=' + item.id, {
                    title: '修改' + options.title,
                        ...
                        const row = admin.storage.getParse(options.key);
                        if (row) {
                            if (admin.debug()) {
            ...
            // 从已有中导入
            select: function () {
                admin.iframe.open(options.prefix + '/select', {
                    title: options.title + '选择',
                    end: () => {
                        const rows = admin.storage.getArray(options.key);
                        if (rows) {
                            if (admin.debug()) {
