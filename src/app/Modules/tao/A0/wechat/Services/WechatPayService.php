...
class WechatPayService
{
    ...
    public static function forceCache(): array
    {
        if ($cache = WechatPayApp::queryBuilder()
            ->where('done', 1)
            ->findColumn(['id', 'app_id', 'mch_id', 'private_key', 'certificate', 'secret_key',
                'v2_secret_key', 'platform_cert'], 'app_id')) {
    ...
    {
        return WechatConfigService::getWith('pay_appid', '');
    }
...
//        Logger::info($data, $postData);
    ...
    {
        $utils = $app->getUtils();
        return $utils->buildBridgeConfig($responseData['prepay_id'], $app->getConfig()->get('appid'));
    }
}